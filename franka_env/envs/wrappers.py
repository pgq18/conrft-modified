...
# pynput is only needed for KeyBoardIntervention (old class)
# KeyBoardIntervention2 uses GLFW instead
try:
    from pynput import keyboard
    PYNPUT_AVAILABLE = True
except (ImportError, OSError):
    # OSError can occur when no X display is available (headless environment)
...
class KeyBoardIntervention(gym.ActionWrapper):
    def __init__(self, env, action_indices=None):
        if not PYNPUT_AVAILABLE:
            raise ImportError(
                "KeyBoardIntervention requires pynput to be installed and an X display to be available. "
                "Please either: 1) Install and configure pynput with an X server, or "
                "2) Use KeyBoardIntervention2 instead (uses GLFW and works better with MuJoCo viewers)."
            )
        super().__init__(env)
